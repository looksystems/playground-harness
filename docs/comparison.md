...
| **LLM Client** | litellm (multi-provider) | OpenAI + Anthropic in-tree via pluggable `LlmClient` interface | OpenAI + Anthropic in-tree via pluggable `ClientInterface` | OpenAI + Anthropic in-tree; pluggable `llm.Client` interface |
...
TypeScript uses a pluggable `LlmClient` interface (in `src/typescript/llm/client.ts`) with two in-tree adapters: `OpenAIClient` (backed by the [`openai`](https://github.com/openai/openai-node) SDK) and `AnthropicClient` (backed by [`@anthropic-ai/sdk`](https://github.com/anthropics/anthropic-sdk-typescript)). The Anthropic adapter handles the system-message extraction, role mapping, tool-definition translation, and tool-call delta accumulation transparently. Pick a provider via `provider: "openai" | "anthropic"` (default openai) on the agent constructor, or inject a pre-built client via `client:`.

PHP uses a pluggable `ClientInterface` (in `src/php/Llm/ClientInterface.php`) with two in-tree adapters: `OpenAIClient` (backed by [`openai-php/client`](https://github.com/openai-php/client)) and `AnthropicClient` (backed by the official [`anthropic-ai/sdk`](https://github.com/anthropics/anthropic-sdk-php)). Same translation responsibilities and same provider-switch pattern as TypeScript: `->provider('openai'|'anthropic')` on the builder (default openai) or `->llmClient(...)` to inject a pre-built one. The legacy `client:` constructor parameter (raw `OpenAI\Contracts\ClientContract`) still works for backward compat.
